...
- **当前页面一键收藏**：点击浏览器工具栏按钮、按 `Alt Shift S`，或右键把当前页面/链接直接收藏到收集箱或指定标签；
- **收集后集中整理**：在收集箱中多选链接，一次移动到目标文件夹；
- **打开优先，整理按需**：日常浏览保持简洁，编辑、拖动排序和文件夹管理集中在“整理书签”模式；
- **搜索与命令面板**：按 `⌘ K` 搜索链接和标签，输入 `>` 可执行新建、切换视图、批量打开、回收站等命令；
- **智能视图**：动态查看最近添加、久未访问、信息待修复和重复链接，不改变原有文件夹归属；
- **重复链接合并**：选择主记录，补齐元数据并合并点击记录，其余副本进入可恢复的回收站；
- **链接维护中心**：主动检查失效、暂不可达和永久重定向，并安全更新 301/308 跳转后的网址；
- **全局置顶**：把少量高频链接固定在主页顶部，不复制链接，也不改变原标签归属；
- **版本快照**：每天自动保存一次，也可手动创建并恢复标签树、链接、排序和显示设置；
- **树内检索与统计**：搜索标签时保留祖先路径，并显示每个节点的直属链接数和子树总数；
- **自动归集**：给文件夹设置 `github.com`、`*.github.com` 一类域名规则，已有和新建链接都会自动归类；
- **回收站与撤销**：删除标签或链接后可立即撤销，也可稍后按原层级和位置恢复；
- **批量打开**：预览并打开当前标签或整个标签子树，可选择当前窗口或新窗口；
- **网页信息补全**：自动获取标题、描述和站点图标；失败时仍可正常保存、打开和手动重试；
- **个性化外观**：支持浅色、深色和跟随系统，以及列数、卡片宽度、间距、字体、强调色等设置；
- **离线只读快照**：本机服务暂时不可用时，仍可查看最近一次成功同步的内容；
- **离线收藏队列**：服务临时离线时先暂存当前页面，恢复连接后自动写入；
...
建议把 Local Speed Dial 固定到 Chrome 工具栏。浏览网页时点击扩展按钮，当前页面会进入“收集箱”；回到新标签页后点击“整理收集箱”，即可多选并批量归类。重复页面不会再次创建。
