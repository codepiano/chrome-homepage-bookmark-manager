...
function App() {
  ...
  const rootStyle = useMemo(() => ({ '--accent': settings.accentColor, ...(settings.textColor ? { '--text': settings.textColor } : {}), '--card-width': `${settings.cardWidth}px`, '--gap': `${settings.gap}px`, '--columns': String(settings.columns), '--grid-justify': settings.centered ? 'center' : 'start', fontFamily: settings.fontFamily } as React.CSSProperties), [settings]);
  ...
  async function load() {
    ...
    try {
      ...
      setFolders(nextFolders); setSettings({ ...defaultSettings, ...nextSettings });
      ...
      if (snapshot) {
        setFolders(snapshot.folders); setSettings({ ...defaultSettings, ...snapshot.settings });
...
function SettingsDialog({ settings, onClose, onOpenConnection, onSaved }: { settings: Settings; onClose(): void; onOpenConnection(): void; onSaved(settings: Settings): void }) {
  ...
  return <DialogFrame title="显示设置" onClose={onClose}><div className="form settings"><fieldset><legend>布局</legend><label>视图<select value={draft.layout} onChange={(event) => patch('layout', event.target.value as Settings['layout'])}><option value="grid">文字卡片</option><option value="list">列表</option></select></label><label>主题<select value={draft.theme} onChange={(event) => patch('theme', event.target.value as Settings['theme'])}><option value="system">跟随系统</option><option value="light">浅色</option><option value="dark">深色</option></select></label><label>列数<input type="number" min="1" max="8" value={draft.columns} onChange={(event) => patch('columns', Number(event.target.value))} /></label><label>卡片宽度<input type="number" min="175" max="600" value={draft.cardWidth} onChange={(event) => patch('cardWidth', Number(event.target.value))} /></label><label>卡片间距<input type="number" min="4" max="40" value={draft.gap} onChange={(event) => patch('gap', Number(event.target.value))} /></label><label>字体<select value={draft.fontFamily} onChange={(event) => patch('fontFamily', event.target.value)}><option value="system-ui">系统默认</option><option value="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif">系统无衬线</option><option value="Georgia, serif">衬线</option><option value="monospace">等宽</option></select></label><label>正文颜色<input type="color" value={draft.textColor || '#202124'} onChange={(event) => patch('textColor', event.target.value)} /></label><label>强调颜色<input type="color" value={draft.accentColor} onChange={(event) => patch('accentColor', event.target.value)} /></label><label className="check"><input type="checkbox" checked={draft.centered} onChange={(event) => patch('centered', event.target.checked)} />卡片区域居中</label></fieldset><fieldset><legend>显示内容</legend>{([{ key: 'compact', label: '紧凑布局' }, { key: 'showDescription', label: '显示简介' }, { key: 'showClickCount', label: '显示访问次数' }, { key: 'showLastVisited', label: '显示最近访问' }] as const).map(({ key, label }) => <label className="check" key={key}><input type="checkbox" checked={draft[key]} onChange={(event) => patch(key, event.target.checked)} />{label}</label>)}</fieldset><section className="connection-panel"><div><strong>本机服务</strong><p>服务地址和配对令牌仅在连接异常或重新配对时需要。</p></div><button className="quiet-button" onClick={onOpenConnection}>管理连接</button></section>{message && <p className="form-error">{message}</p>}<footer><button className="primary" onClick={() => void save()}>保存显示设置</button></footer></div></DialogFrame>;
}
