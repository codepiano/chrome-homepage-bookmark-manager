...
function SmartViewPanel({ view, folders, settings, readOnly, onRecord, onRetry, onEdit }: { view: SmartView; folders: Folder[]; settings: Settings; readOnly: boolean; onRecord(link: Link): void; onRetry(link: Link): void; onEdit(link: Link): void }) {
  return <div className="smart-view-panel"><div className="section-heading"><div><h2>{view.label}</h2><p>{view.links.length} 个链接 · {view.description}{readOnly ? ' · 离线快照只读' : ''}</p></div></div>{view.links.length ? <div className={`links ${settings.layout} columns-${settings.columnMode}`}>{view.links.map((link) => <SmartViewCard key={link.id} link={link} folderName={folders.find((folder) => folder.id === link.folderId)?.name ?? '未知标签'} settings={settings} readOnly={readOnly} onRecord={() => onRecord(link)} onRetry={() => onRetry(link)} onEdit={() => onEdit(link)} />)}</div> : <div className="smart-view-empty"><strong>这里暂时是空的</strong><p>{view.emptyMessage}</p></div>}</div>;
}
...
function App() {
  ...
  return <main className={`app theme-${settings.theme} ${settings.compact ? 'compact' : ''} mode-${mode}`} style={rootStyle}>
    <header className="header"><h1>快速访问</h1><BookmarkSearch query={searchQuery} links={allLinks} folders={folders} onQueryChange={setSearchQuery} onOpen={openLink} /><div className="header-actions"><button className={`quiet-button history-button ${showHistory ? 'active' : ''}`} disabled={batchMoving} aria-pressed={showHistory} onClick={() => { setShowHistory(true); setSmartViewId(null); setMode('browse'); }}>浏览记录</button><button className="quiet-button" disabled={batchMoving} onClick={() => setDialog({ type: 'settings' })}>设置</button>{!offlineSnapshot && <button className={mode === 'organize' ? 'primary' : 'quiet-button'} disabled={batchMoving} onClick={() => { setShowHistory(false); setSmartViewId(null); setMode((current) => current === 'browse' ? 'organize' : 'browse'); }}>{mode === 'organize' ? '完成整理' : !smartViewId && isInbox && links.length ? '整理收集箱' : '整理书签'}</button>}</div></header>
    ...
    {tabs}
    {!loading && <SmartViewBar views={smartViews} activeId={smartViewId} disabled={batchMoving} onSelect={(id) => { setSmartViewId(id); setShowHistory(false); setMode('browse'); }} />}
    <section className="content" aria-busy={loading}>{loading ? <p className="state">正在连接本机书签库…</p> : showHistory ? <HistoryPanel onOpen={(url) => window.location.assign(url)} onAdd={(item) => selectedFolderId ? setDialog({ type: 'link', initial: { url: item.url, title: item.title } }) : setToast('请先创建一个标签，再将记录添加为书签。')} /> : activeSmartView ? <SmartViewPanel view={activeSmartView} folders={folders} settings={settings} readOnly={offlineSnapshot} onRecord={recordLinkClick} onRetry={(link) => void retryMetadata(link)} onEdit={(link) => setDialog({ type: 'link', link })} /> : !selectedFolder ? <Empty onAdd={() => setDialog({ type: 'folder' })} /> : <>
